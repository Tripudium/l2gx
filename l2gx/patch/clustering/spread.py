...
def spread_clustering_with_stats(
    graph: Graph, num_clusters: int, max_degree_init=True, verbose=True
):
    """
...
def spread_clustering_balanced(
    graph: Graph, num_clusters: int, max_imbalance=1.2, verbose=True
):
    """
            ...
            for over_c in oversized_clusters:
                ...
                if excess > 0 and len(undersized_clusters) > 0:
                    ...
                    for i, node in enumerate(nodes_to_move):
                        target_cluster = undersized_clusters[
                            i % len(undersized_clusters)
                        ]
    ...
    if verbose:
        ...
        print(
            f"  Imbalance ratio: {final_sizes.max().float() / final_sizes.mean():.2f}"
        )
