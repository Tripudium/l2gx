...
def evaluate_clustering(clusters, graph=None, ground_truth=None):
    ...
    if graph is not None:
        ...
        metrics.update({
            ...
            'cut_ratio': edge_cuts.sum().item() / graph.num_edges if hasattr(graph, 'num_edges') else None
        })
...
def compute_edge_cuts(clusters, graph):
    ...
    if cut_edges.any():
        ...
        cuts_per_cluster = torch.bincount(cut_src_clusters, minlength=len(cuts_per_cluster))
...
def compare_clusterings(clusters1, clusters2):
    ...
    try:
        ...
        clusters1_np = clusters1.cpu().numpy() if torch.is_tensor(clusters1) else np.array(clusters1)
        clusters2_np = clusters2.cpu().numpy() if torch.is_tensor(clusters2) else np.array(clusters2)
        
        return {
            'adjusted_rand_score': adjusted_rand_score(clusters1_np, clusters2_np),
            'normalized_mutual_info': normalized_mutual_info_score(clusters1_np, clusters2_np)
        }
    except ImportError:
...
def convert_graph_format(graph, target_format):
    ...
    if target_format == 'tgraph':
        if hasattr(graph, 'edge_index'):
            ...
            raise NotImplementedError(f"Conversion from {type(graph)} to TGraph not implemented")
        ...
        if hasattr(graph, 'edge_index'):
            ...
            raise NotImplementedError(f"Conversion from {type(graph)} to PyG not implemented")
        ...
        if hasattr(graph, 'edge_index'):
            ...
            raise NotImplementedError(f"Conversion from {type(graph)} to NetworkX not implemented")
...
def validate_clustering_result(clusters, num_nodes, num_clusters=None):
    ...
    sizes = torch.bincount(clusters[clusters >= 0]) if len(unique_clusters) > 0 else torch.tensor([])
