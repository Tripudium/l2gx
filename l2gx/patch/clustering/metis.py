...
def metis_clustering(graph: TGraph, num_clusters: int):
    ...
    try:
        ...
        cluster_tensor = torch.as_tensor(
            memberships, dtype=torch.long, device=graph.device
        )
...
def metis_clustering_weighted(
    graph: TGraph, num_clusters: int, node_weights=None, edge_weights=None, verbose=True
):
    """
    ...
    try:
        ...
        if verbose:
            cluster_sizes = torch.bincount(cluster_tensor, minlength=num_clusters)
            print(
                f"Weighted METIS clustering: {num_clusters} clusters, "
