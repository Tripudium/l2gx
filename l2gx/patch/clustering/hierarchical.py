...
def hierarchical_clustering(
    ...
    def recursive_clustering(data, m, k):
        ...
        unique_clusters, cluster_counts = torch.unique(
            cluster_tensor, return_counts=True
        )
        ...
        for cluster_id, count in zip(unique_clusters, cluster_counts):
            ...
            if count <= k:
                ...
                max_cluster_id = (
                    max(unique_clusters).item() if len(unique_clusters) > 0 else -1
                )
    ...
    if branch_factors is None:
        ...
        if not isinstance(branch_factors, Iterable):
            ...
            if levels is None:
                ...
                raise ValueError(
                    f"levels={levels} does not match {len(branch_factors)=}"
                )
...
def adaptive_hierarchical_clustering(
    ...
    while level < max_levels:
        ...
        print(
            f"Level {level}: {current_graph.num_nodes} nodes, "
        ...
        print(
            f"  Cluster sizes: [{min_size}, {max_size}], "
    ...
    return {
        ...
        "final_max_size": final_cluster_sizes.max().item()
        if len(final_cluster_sizes) > 0
        else 0,
        "final_min_size": final_cluster_sizes.min().item()
        if len(final_cluster_sizes) > 0
        else 0,
    }
