...
def _simplified_svd(matrix, k, random_state=None):
    ...
    try:
        ...
        U, s, Vt = np.linalg.svd(matrix.toarray() if hasattr(matrix, 'toarray') else matrix)
        return U[:, :k], s[:k], Vt[:k, :]
...
class SVDEmbedding(TransductiveGraphEmbedding):
    ...
    def fit(self, graph: Union[Data, nx.Graph, sp.spmatrix, np.ndarray]) -> 'SVDEmbedding':
        """
        ...
        if self.matrix_type == 'adjacency':
            ...
            degree = np.array(adj_matrix.sum(axis=1)).flatten()
            degree_inv_sqrt = np.power(degree, -0.5, out=np.zeros_like(degree), where=(degree != 0))
        ...
        # Perform SVD
        U, s, _ = _simplified_svd(matrix, k=self.embedding_dim, random_state=self.random_state)
