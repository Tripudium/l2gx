...
class PatchedEmbedding(GraphEmbedding):
    ...
    def __init__(
        ...
        **embedding_kwargs
        ...
        if alignment_method != 'l2g':
            raise ValueError(f"Only 'l2g' alignment method is currently supported, got '{alignment_method}'")
    ...
    def fit(self, graph: Union[Data, nx.Graph, sp.spmatrix, np.ndarray]) -> 'PatchedEmbedding':
        """
        ...
        if self.verbose:
            print(f"Generating {self.num_patches} patches using {self.clustering_method} clustering...")
            
        self._patches, self._patch_graph = generate_patches(
            self._tgraph,
        ...
        if self.verbose:
            ...
            print(f"Created {len(self._patches)} patches with sizes: "
        ...
        for i, patch in enumerate(self._patches):
            ...
            import torch
            patch_nodes = torch.tensor(patch.nodes, dtype=torch.long, device=self._tgraph.device)
    ...
    def get_patches(self) -> Optional[list]:
        """
    ...
    def get_patch_graph(self) -> Optional[object]:
        """
    ...
    def set_params(self, **params) -> 'PatchedEmbedding':
        ...
        embedding_specific = [
            'embedding_method', 'num_patches', 'clustering_method', 'alignment_method',
            'min_overlap', 'target_overlap', 'sparsify_method', 'target_patch_degree', 
            'enable_scaling', 'verbose'
        ]
