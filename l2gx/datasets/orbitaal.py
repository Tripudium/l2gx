...
class ORBITAALDataset(BaseDataset):
    ...
    def download(self):
        ...
        if self.subset == "sample":
            ...
            for name, url in self.urls.items():
                file_path = (
                    raw_dir / f"{name}_data.zip"
                    if name == "sample"
                    else raw_dir / Path(url).name
                )
            ...
            with open(info_file, "w") as f:
                ...
                f.write("=================================\n\n")
                f.write(
                    "The full ORBITAAL dataset is very large (>1TB) and must be downloaded manually.\n\n"
                )
                ...
                f.write(
                    "1. Visit: https://www.cs.cornell.edu/~arb/data/temporal-bitcoin/\n"
                )
    ...
    def process(self):
        ...
        if self.subset == "sample":
            ...
            if (
                sample_zip.exists() and sample_zip.stat().st_size > 100
                ...
                try:
                    ...
                    self.logger.warning(
                        "Sample zip file is corrupted, creating synthetic data"
                    )
    ...
    def _create_synthetic_sample(self):
        ...
        self.logger.info(
            f"Created synthetic sample: {num_entities} entities, {num_transactions} transactions"
        )
    ...
    def _process_full_dataset(self, raw_dir: Path):
        ...
        self.logger.warning(
            "Full dataset processing not implemented - using synthetic sample"
        )
        self._create_synthetic_sample()
