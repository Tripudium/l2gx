...
def transform_patches(
    ...
    if scale_range is not None:
        scales = np.exp(
            rg.uniform(np.log(scale_range[0]), np.log(scale_range[1]), size=n_patches)
        )
        scales[0] = 1.0
    else:
...
def generate_patch_graph(
    ...
    """
    Generate a patch graph from a set of points.

    Returns a tuple of (patches, patch_graph) where patch_graph is a TGraph
    with patches and overlap_nodes attributes for alignment.
    """
    patches, centers = voronoi_patches(
        points, sample_size, min_degree, min_overlap, min_patch_size, eps, kmeans
    )

    # Build edge list for patch graph based on overlaps
    edges = []
    overlap_nodes_dict = {}

    for i in range(len(patches)):
        for j in range(i + 1, len(patches)):
            # Find overlapping nodes between patches
            nodes_i = set(patches[i].nodes)
            nodes_j = set(patches[j].nodes)
            overlap = list(nodes_i & nodes_j)

            # Only add edge if overlap is sufficient
            if len(overlap) >= min_overlap:
                edges.append([i, j])
                edges.append([j, i])  # Add reverse edge for undirected graph
                overlap_nodes_dict[(i, j)] = overlap
                overlap_nodes_dict[(j, i)] = overlap  # Add reverse direction
    ...
    # Add required attributes to patch_graph
    patch_graph.patches = patches
    patch_graph.overlap_nodes = overlap_nodes_dict

    # Print summary
    num_edges = patch_graph.num_edges // 2  # Divide by 2 for undirected edges
    avg_overlap = (
        np.mean([len(nodes) for nodes in overlap_nodes_dict.values()])
        if overlap_nodes_dict
        else 0
    )
    print(f"Created patch graph with {len(patches)} patches, {num_edges} edges")
    print(f"Average overlap size: {avg_overlap:.1f} nodes")
