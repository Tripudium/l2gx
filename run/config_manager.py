...
class PatchConfig:
    ...
    sparsification_params: Dict[str, Any] = field(default_factory=lambda: {
...
class NodeReconstructionConfig:
    ...
    evaluation_metrics: List[str] = field(default_factory=lambda: ["mse", "mae", "cosine_similarity"])
...
class NodeClassificationConfig:
    ...
    rf_max_depth: int = 10
    evaluation_metrics: List[str] = field(default_factory=lambda: ["accuracy", "f1_macro", "f1_micro", "precision", "recall"])
    stratify: bool = True
...
class Config:
    ...
    node_reconstruction: NodeReconstructionConfig = field(default_factory=NodeReconstructionConfig)
    node_classification: NodeClassificationConfig = field(default_factory=NodeClassificationConfig)
...
class ConfigManager:
    ...
    def _dict_to_config(self, config_dict: Dict[str, Any]) -> Config:
        ...
        if 'node_reconstruction' in config_dict:
            config.node_reconstruction = NodeReconstructionConfig(**config_dict['node_reconstruction'])
        
        if 'node_classification' in config_dict:
            config.node_classification = NodeClassificationConfig(**config_dict['node_classification'])
        
        if 'hyperparameter_search' in config_dict:
            config.hyperparameter_search = HyperparameterSearchConfig(**config_dict['hyperparameter_search'])
    ...
    def _validate_config(self, config: Config):
        ...
        if config.embedding.method not in self.VALID_EMBEDDING_METHODS:
            raise ValueError(f"Invalid embedding method: {config.embedding.method}. "
        ...
        if config.patches.clustering_method not in self.VALID_CLUSTERING_METHODS:
            raise ValueError(f"Invalid clustering method: {config.patches.clustering_method}. "
        ...
        if config.alignment.method not in self.VALID_ALIGNMENT_METHODS:
            raise ValueError(f"Invalid alignment method: {config.alignment.method}. "
                           f"Valid options: {self.VALID_ALIGNMENT_METHODS}")
        
        # Validate device
        if config.experiment.device not in self.VALID_DEVICES:
            raise ValueError(f"Invalid device: {config.experiment.device}. "
                           f"Valid options: {self.VALID_DEVICES}")
        
        # Validate split ratios
        total_ratio = config.dataset.train_ratio + config.dataset.val_ratio + config.dataset.test_ratio
    ...
    def get_experiment_variants(self) -> List[Config]:
        ...
        if search_config.method == "grid":
            ...
            return self._generate_random_search_configs(search_space, search_config.n_trials)
        else:
            # For Optuna, return base config (Optuna will handle parameter generation)
            return [self.config]
    
    def _generate_grid_search_configs(self, search_space: Dict[str, List[Any]]) -> List[Config]:
    ...
    def _generate_random_search_configs(self, search_space: Dict[str, List[Any]], n_trials: int) -> List[Config]:
